...
final class InstallmentsController
{
    public function index(Request $request): never
    {
        ...
        $stmt = $this->prepare($conn, 'SELECT i.installment_id, i.installment_no, i.loan_id, i.due_amount, i.paid_amount, i.status, i.paid_date, l.loan_code, l.member_id, m.full_name, m.member_code FROM installments i LEFT JOIN loans l ON i.loan_id = l.loan_id LEFT JOIN members m ON l.member_id = m.member_id' . $where . ' ORDER BY i.installment_no ASC, i.installment_id ASC LIMIT ? OFFSET ?', $listTypes, $listValues);
        $stmt->execute();
    ...
    public function show(Request $request): never
    {
        ...
        $conn = Database::connection();
        $stmt = $conn->prepare('SELECT i.installment_id, i.installment_no, i.loan_id, i.due_amount, i.paid_amount, i.status, i.paid_date, l.loan_code, l.member_id, m.full_name, m.member_code FROM installments i LEFT JOIN loans l ON i.loan_id = l.loan_id LEFT JOIN members m ON l.member_id = m.member_id WHERE i.installment_id = ? LIMIT 1');
    ...
    public function update(Request $request): never
    {
        ...
        $stmt = $conn->prepare('UPDATE installments SET paid_amount = ?, status = ?, paid_date = ? WHERE installment_id = ?');
        $stmt->bind_param('dssi', $paidAmount, $status, $paidDate, $id);
        $stmt->execute();
        $stmt->close();
